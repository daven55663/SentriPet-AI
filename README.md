...
- 每週／每月額度快重置、卻還剩不少時，桌寵會拿鬧鐘催你把它用掉，不然就浪費了：

| 距離重置 | 還剩 | 桌寵的反應 |
|---|---|---|
| 2 天內 | 30% 以上 | 黃色提醒標籤＋鬧鐘，約每小時提醒一次 |
| 最後一天 | 10% 以上 | 橘色標籤，鬧鐘會響，約每 25 分鐘提醒一次 |
| 最後 6 小時 | 5% 以上 | 紅色標籤閃爍，鬧鐘狂響，約每 12 分鐘提醒一次 |

每升一級會跳一次 Windows 通知（重開機也不會重複跳）。AI 正在工作時不會打擾你；5 小時額度用完時也先不催（反正用不了）。
右鍵選單「催我用完週額度」或 設定 → 提醒 可以關掉。
