...
{
    /// <summary>
    /// (Avalonia port of src/Themes/PetTheme.cs.)
    ...
    class PetTheme : Theme
    {
        ...
        class Card
        {
            ...
            public Control AccBlink, AccGlow;
        ...
        {
            return BoundsOf(row, relativeTo) ?? base.ContentBounds(relativeTo);
        }
        ...
        Card MakeCard(ProviderView v)
        {
            ...
            c.Color = v.HasData ? v.Color.ToColor() : G.Desaturate(v.Color, 0.6);
            var color = c.Color;
            ...
            c.ShadowScale = new ScaleTransform(1, 1);
            c.Shadow.RenderTransformOrigin = G.At(32, 5);
            ...
            c.BodyScale = new ScaleTransform(1, 1);
            c.Body.RenderTransformOrigin = G.At(BodyW / 2, BodyH);
            ...
            c.Outline = new Path { Data = BodyGeo, Stroke = G.B(dark), StrokeThickness = 2.4, StrokeJoin = PenLineJoin.Round };
            ...
            foreach (var e in new Control[] { c.HappyL, c.HappyR, c.SleepL, c.SleepR, c.BrowL, c.BrowR, c.Mouth })
            ...
            c.Question = G.T("?", 20, dark, FontWeight.Black, G.Num);
            G.Place(c.Question, CardW / 2 + 22, 4);
            c.Fx.Children.Add(c.Question);
            // "!" when the alarm clock rings
            c.Exclaim = G.T("!", 21, Palette.Hex("#EF4444"), FontWeight.Black, G.Num);
            c.ExclaimPop = new ScaleTransform(1, 1);
            c.Exclaim.RenderTransformOrigin = new RelativePoint(0.5, 1, RelativeUnit.Relative);
            c.Exclaim.RenderTransform = c.ExclaimPop;
            c.Exclaim.IsVisible = false;
            G.Place(c.Exclaim, CardW / 2 + 24, 2);
            c.Fx.Children.Add(c.Exclaim);
            c.Zzz = new TextBlock[3];
            for (int i = 0; i < 3; i++)
            {
                c.Zzz[i] = G.T("z", 10 + i * 3, Palette.Hex("#6B7FD7"), FontWeight.Bold, G.Num);
            ...
            for (int i = 0; i < 4; i++)
            {
                c.Sparkles[i] = new Path { Data = G.Star(new Point(0, 0), 5, 1.6, 4, 0), Fill = G.B(Palette.Hex("#FFD166")), Stroke = G.B(Palette.Hex("#E09F00")), StrokeThickness = 0.8, RenderTransform = new ScaleTransform(1, 1), RenderTransformOrigin = G.At(0, 0) };
        ...
        void ApplyUseIt(Card c, ProviderView v)
        {
            ...
            clock.RenderTransformOrigin = new RelativePoint(0.5, 0.85, RelativeUnit.Relative);
        ...
        static Canvas MakeEye(double cx, double cy, out ScaleTransform scale, out TranslateTransform move)
        {
            ...
            eye.RenderTransformOrigin = G.At(0, 0);
            return eye;
        }

        Control MakeAccessory(Card c, string mascot, Color color, Color dark)
        {
            var acc = new Canvas();
            var stroke = G.B(dark);
            switch (mascot)
            {
                ...
                case "prompt":
                {
                    ...
                    var gt = G.T(">", 10, Palette.Hex("#6EF2A6"), FontWeight.Bold, G.Mono);
                    G.Place(gt, 32, -21.5);
                    acc.Children.Add(gt);
        ...
        Control MakePlate(Card c, ProviderView v)
        {
            ...
            c.NameText = G.T(v.Name, 12, Palette.Hex("#2B2B35"), FontWeight.Bold, G.Ui);
            c.NameText.VerticalAlignment = VerticalAlignment.Center;
            c.PctText = G.T("", 14, Palette.Hex("#16A34A"), FontWeight.Bold, G.Num);
            ...
            if (v.HasData)
            {
                foreach (var m in v.Meters.Take(3))
                {
                    ...
                    var lbl = G.T(m.ShortLabel ?? "", 9, Palette.Hex("#6B7280"), FontWeight.SemiBold, G.Ui);
                    ...
                    r.Pct = G.T("", 9, Palette.Hex("#4B5563"), FontWeight.SemiBold, G.Num);
                ...
                c.ResetText = G.T("", 9, Palette.Hex("#6B7280"), FontWeight.Normal, G.Ui);
            ...
            {
                c.ErrorText = G.T(v.Error ?? L.T("沒有資料"), 9.5, Palette.Hex("#6B7280"), FontWeight.Normal, G.Ui);
        ...
        void ApplyExpression(Card c, string expr)
        {
            ...
            c.EyeL.IsVisible = c.EyeR.IsVisible = open;
            c.HappyL.IsVisible = c.HappyR.IsVisible = expr == "squint";
            c.SleepL.IsVisible = c.SleepR.IsVisible = expr == "sleep";
            bool brows = expr == "worried" || expr == "critical" || hurry;
            c.BrowL.IsVisible = c.BrowR.IsVisible = brows;
            // anxious brows sit higher (alarmed) than the tired "running low" ones
            c.BrowLiftL.Y = c.BrowLiftR.Y = hurry ? -1.5 - urgency * 0.6 : 0;
            c.Sweat.IsVisible = (brows && !hurry) || urgency >= 2;
            c.BlushL.Opacity = c.BlushR.Opacity = (expr == "happy" || expr == "squint") ? 1 : expr == "content" ? 0.55 : 0.2;
            c.Question.IsVisible = expr == "unknown";
            if (!hurry) c.Exclaim.IsVisible = false;
            else c.Exclaim.Foreground = G.B(G.UseItAccent(urgency));
            foreach (var z in c.Zzz) z.IsVisible = expr == "sleep";
        ...
        void TickCard(Card c, double dt)
        {
            ...
            else if (c.Exclaim.IsVisible == true) c.Exclaim.IsVisible = false;
            if (c.Question.IsVisible == true)
            ...
            {
                var sp = c.Sparkles[i];
                if (!sparkle) { sp.IsVisible = false; continue; }
                sp.IsVisible = true;
        ...
        static void Wave(double level, double time, double phase, double amp, out Geometry fill, out Geometry surface)
        {
            ...
            using (var sc = s.Open())
            {
                fc.BeginFigure(new Point(-2, BodyH + 2), true);
                const int n = 22;
                for (int i = 0; i <= n; i++)
                {
                    ...
                    fc.LineTo(pt);
                    if (i == 0) sc.BeginFigure(pt, false);
                    else sc.LineTo(pt);
                }
                fc.LineTo(new Point(BodyW + 2, BodyH + 2));
                fc.EndFigure(true);
                sc.EndFigure(false);
            }
            fill = f;
        ...
        void TickBubble(Card c, double dt)
        {
            ...
            if (c.BubbleAlpha <= 0)
            {
                if (c.Bubble.IsVisible != false)
                {
                    c.Bubble.IsVisible = false;
                    c.BubbleTail.IsVisible = false;
                }
                return;
            }
            c.Bubble.IsVisible = true;
            c.BubbleTail.IsVisible = true;
            ...
            var at = c.Panel.TranslatePoint(new Point(CardW / 2, 0), bubbleLayer);
            if (at == null) return;
            var origin = at.Value;
            double layerW = Math.Max(bubbleLayer.Bounds.Width, size.Width);
